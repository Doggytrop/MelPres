...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (! Schema::hasColumns('payments', ['company_id', 'recorded_by'])
            || ! Schema::hasColumns('users', ['company_id', 'id'])) {
            throw new \RuntimeException(
                'No se puede crear payments_company_recorded_tenant_fk: faltan columnas requeridas.'
            );
        }

        if (! $this->hasExactIndex('payments', ['company_id', 'recorded_by'])) {
            throw new \RuntimeException(
                'No se puede crear payments_company_recorded_tenant_fk: falta el índice hijo.'
            );
        }

        if (! $this->hasExactIndex('users', ['company_id', 'id'], true)) {
            throw new \RuntimeException(
                'No se puede crear payments_company_recorded_tenant_fk: falta el UNIQUE padre.'
            );
        }
