...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (! Schema::hasColumns('restructurings', ['company_id', 'original_loan_id'])
            || ! Schema::hasColumns('loans', ['company_id', 'id'])) {
            throw new \RuntimeException(
                'No se puede crear restruct_original_loan_tenant_fk: faltan columnas requeridas.'
            );
        }
        ...
        if (! $this->hasExactIndex(
            ...
            throw new \RuntimeException(
                'No se puede crear restruct_original_loan_tenant_fk: falta el índice hijo.'
            );
        }

        if (! $this->hasExactIndex('loans', ['company_id', 'id'], true)) {
            throw new \RuntimeException(
                'No se puede crear restruct_original_loan_tenant_fk: falta el UNIQUE padre.'
            );
        }
