...
return new class extends Migration
{
    ...
    private string $loanTenantForeignKey = 'payments_company_loan_tenant_fk';

    private string $recordedTenantForeignKey = 'payments_company_recorded_tenant_fk';
        ...
        if ($columnState === 'initial') {
            try {
                DB::statement(sprintf(
                    ...
                    $this->quoteIdentifier($this->paymentsTable),
                    $this->quoteIdentifier('company_id')
                ));
            } catch (Throwable $exception) {
                throw new RuntimeException(
                    'MySQL could not make payments.company_id NOT NULL while '
                    .'the verified composite foreign keys remained installed. '
                    .'No composite foreign key or index was removed; the missing '
                    .'simple company foreign key is a recoverable partial state.',
                    0,
                    $exception
                );
            }
    ...
    public function down(): void
    {
        ...
        if ($columnState === 'final') {
            try {
                ...
                throw new RuntimeException(
                    'MySQL could not restore nullable payments.company_id while '
                    .'the verified composite foreign keys remained installed. '
                    .'No composite foreign key or index was removed.',
                    0,
                    $exception
                );
            }
    ...
    private function assertMySql(): void
    {
        if (DB::getDriverName() !== 'mysql') {
            throw new RuntimeException(
                'Hardening payments.company_id requires MySQL.'
            );
        }
    }
    ...
    {
        foreach ([
            $this->paymentsTable => ['id', 'company_id', 'loan_id', 'recorded_by'],
        ...
        if (! $this->isBigIntUnsigned($companyId)
            ...
            throw new RuntimeException(
                'payments.company_id and companies.id must both be BIGINT UNSIGNED.'
            );
        }
    }
    ...
    {
        $nullIds = DB::table($this->paymentsTable)
        ...
        if ($nullIds->isNotEmpty()) {
            throw new RuntimeException(
                'payments.company_id contains NULL values for IDs: '
                .$nullIds->implode(', ').'.'
            );
        }
    ...
    {
        $orphanIds = DB::table("{$this->paymentsTable} as payment")
            ->leftJoin(
            ...
            ->whereNotNull('payment.company_id')
        ...
        if ($orphanIds->isNotEmpty()) {
            throw new RuntimeException(
                'Orphan company references found in payments: '
                .$orphanIds->implode(', ').'.'
            );
        }
    }
    ...
    private function assertTenantRelationIsConsistent(
        ...
        $query = DB::table("{$this->paymentsTable} as payment")
            ->leftJoin(
        ...
        if ($nullable) {
            $query->whereNotNull("payment.{$foreignId}");
        }
        ...
        if ($orphanIds->isNotEmpty()) {
            throw new RuntimeException(
                "Payments with missing {$label} references found: "
                .$orphanIds->implode(', ').'.'
            );
        }
        ...
        $crossCompanyIds = (clone $query)
            ...
            ->whereRaw('NOT (payment.company_id <=> parent.company_id)')
            ->orderBy('payment.id')
            ->pluck('payment.id');

        if ($crossCompanyIds->isNotEmpty()) {
            throw new RuntimeException(
                "Cross-company payment/{$label} references found: "
    ...
    private function columnState(): string
    {
        ...
        throw new RuntimeException(
            "Unexpected payments.company_id definition: {$definition}."
        );
    }
    ...
    private function companyIdDefinition(): string
    {
        $row = (array) DB::selectOne(
            'SHOW CREATE TABLE '.$this->quoteIdentifier($this->paymentsTable)
        );
        ...
        if (! isset($values[1])
            ...
            throw new RuntimeException(
                'Unable to inspect payments.company_id with SHOW CREATE TABLE.'
            );
        }
    ...
    private function companyForeignKey(): ?array
    {
        $foreignKeys = array_values(array_filter(
            Schema::getForeignKeys($this->paymentsTable),
        ...
        if (count($foreignKeys) > 1) {
            throw new RuntimeException(
                'More than one simple foreign key uses payments.company_id.'
            );
        }
        ...
        if (strcasecmp(
            ...
            throw new RuntimeException(
                'The simple foreign key using payments.company_id has an unexpected target.'
            );
        }
    ...
    private function foreignKeyState(?array $foreignKey): string
    {
        ...
        throw new RuntimeException(
            'The payments.company_id foreign key has unexpected update/delete rules.'
        );
    }

    private function compositeForeignKeySnapshot(): array
    {
        $expected = [
        ...
        foreach ($expected as $name => $definition) {
            ...
            if (count($matching) !== 1) {
                throw new RuntimeException(
                    "Expected composite foreign key {$name} exactly once."
                );
            }
            ...
            if ($this->lowercaseColumns($foreignKey['columns'] ?? [])
                ...
                throw new RuntimeException(
                    "Composite foreign key {$name} has an unexpected definition."
                );
            }
    ...
    private function assertConstraintNameCanBeUsed(
        ...
        $conflict = DB::table('information_schema.TABLE_CONSTRAINTS')
            ...
            ->whereRaw('LOWER(TABLE_NAME) <> ?', [strtolower($this->paymentsTable)])
            ->exists();
    ...
    {
        $name = strtolower($this->paymentsTable.'_company_id_foreign');

        if (strlen($name) > 64) {
            throw new RuntimeException(
                'The conventional payments.company_id foreign key name exceeds 64 characters.'
            );
        }
    ...
    private function assertFinalState(string $foreignKeyName, array $snapshot): void
    {
        if ($this->columnState() !== 'final') {
            throw new RuntimeException(
                'payments.company_id did not reach BIGINT UNSIGNED NOT NULL without a default.'
            );
        }
    ...
    private function assertInitialState(string $foreignKeyName, array $snapshot): void
    {
        if ($this->columnState() !== 'initial') {
            throw new RuntimeException(
                'payments.company_id was not restored as BIGINT UNSIGNED NULL DEFAULT NULL.'
            );
        }
    ...
    private function assertExpectedForeignKey(string $name, string $state): void
    {
        ...
        if ($foreignKey === null
            ...
            throw new RuntimeException(
                "payments.company_id did not reach the expected {$state} foreign key state."
            );
        }
    }
    ...
    private function snapshot(): array
    {
        return [
            ...
            'composite_foreign_keys' => $this->compositeForeignKeySnapshot(),
        ];
    }

    private function rowSnapshot(): array
    {
        $rows = DB::table($this->paymentsTable)
            ->orderBy('id')
            ->get()
    ...
    private function otherColumnSnapshot(): array
    {
        return DB::table('information_schema.COLUMNS')
            ...
            ->where('TABLE_NAME', $this->paymentsTable)
    ...
    private function indexSnapshot(): array
    {
        ...
        }, Schema::getIndexes($this->paymentsTable));
    ...
    private function assertSnapshotUnchanged(array $snapshot): void
    {
        if ($this->rowSnapshot() !== $snapshot['rows']) {
            throw new RuntimeException(
                'Payment rows changed while hardening company_id.'
            );
        }

        if ($this->otherColumnSnapshot() !== $snapshot['other_columns']) {
            throw new RuntimeException(
                'A payments column other than company_id changed.'
            );
        }

        if ($this->indexSnapshot() !== $snapshot['indexes']) {
            throw new RuntimeException(
                'Payment indexes changed while hardening company_id.'
            );
        }

        if ($this->compositeForeignKeySnapshot()
            !== $snapshot['composite_foreign_keys']) {
            throw new RuntimeException(
                'A payments tenant foreign key changed.'
            );
        }
    }
    ...
    private function column(string $name): array
    {
        ...
        if (! is_array($column)) {
            throw new RuntimeException(
                "Unable to inspect {$this->paymentsTable}.{$name}."
            );
        }
