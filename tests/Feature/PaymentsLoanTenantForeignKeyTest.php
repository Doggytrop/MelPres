...
class PaymentsLoanTenantForeignKeyTest extends TestCase
{
    ...
    public function test_composite_foreign_key_has_the_expected_definition(): void
    {
        ...
        $this->assertSame(['company_id', 'loan_id'], $foreignKey['columns']);
    ...
    public function test_valid_payment_from_the_same_company_is_accepted(): void
    {
    ...
    public function test_payment_from_a_different_company_is_rejected(): void
    {
        ...
        $this->insertPayment($otherCompanyId, $loanId, $otherUserId);
    }

    public function test_referenced_loan_cannot_be_deleted(): void
    {
        [$companyId, $loanId, $userId] = $this->tenantLoan('Empresa Uno', 'empresa-uno');
        $this->insertPayment($companyId, $loanId, $userId);
    ...
    public function test_rollback_removes_only_composite_foreign_key(): void
    {
        $migration = require database_path(
            'migrations/2026_07_21_000012_add_payments_loan_tenant_foreign_key.php'
        );

        $migration->down();

        $this->assertNull($this->foreignKey(self::CONSTRAINT));
        $this->assertNotNull($this->foreignKey('payments_loan_id_foreign'));
        $this->assertNotNull(
            $this->namedIndex('payments', 'payments_company_loan_idx')
        );
        $this->assertNotNull(
