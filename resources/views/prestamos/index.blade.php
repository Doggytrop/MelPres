...
<div id="modalPago" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%;
    ...
    <div class="bg-white rounded-3 p-4" style="width:100%; max-width:520px; margin:1rem;">
        ...
        {{-- Formulario de pago (se muestra al seleccionar cliente) --}}
...
function buscarCliente(q) {
    ...
    busquedaTimeout = setTimeout(() => {
        fetch(`/prestamos/buscar-cliente?q=${encodeURIComponent(q)}`)
            .then(r => r.json())
            .then(clientes => mostrarResultados(clientes));
    }, 300);
}
...
function mostrarResultados(clientes) {
    ...
    lista.innerHTML = clientes.map(c => `
        <div class="p-3 rounded-3 mb-2" style="border:0.5px solid #eee; cursor:pointer;"
            ...
            <p class="fw-medium mb-1" style="font-size:13px; color:#1a2e1a;">${c.nombre}</p>
            <p class="mb-2 text-muted" style="font-size:11px;">${c.telefono}</p>
            ${c.prestamos.length
                ? c.prestamos.map(p => `
                    <div class="d-flex justify-content-between align-items-center py-1"
                         style="border-top:0.5px solid #f5f5f5; cursor:pointer;"
                         onclick="seleccionarPrestamo('${c.nombre}', ${p.id}, '${p.tipo}', '${p.saldo}', '${p.mora}', '${p.url}')">
                        <span style="font-size:12px; color:#555;">Préstamo #${p.id} — ${p.tipo}</span>
                        <span style="font-size:12px; color:#1f6b21; font-weight:500;">Saldo: $${p.saldo}</span>
                    </div>
                `).join('')
...
function seleccionarPrestamo(nombre, prestamoId, tipo, saldo, mora, url) {
    document.getElementById('resultadosBusqueda').style.display = 'none';
    ...
    document.getElementById('infoPrestamo').textContent =
        `Préstamo #${prestamoId} — ${tipo} | Saldo: $${saldo}` +
