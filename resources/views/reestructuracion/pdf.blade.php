...
<head>
    <meta charset="UTF-8">
    <title>Acuerdo de Reestructuración #{{ $reestructuracion->id }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-right">
            <div class="folio">Folio #{{ str_pad($reestructuracion->id, 6, '0', STR_PAD_LEFT) }}</div>
            <div class="fecha">Fecha de emisión: {{ $reestructuracion->created_at->format('d/m/Y') }}</div>
        </div>
    </div>
    ...
    <div class="body">
        ...
        <div class="section">
            ...
            <p>
                ...
                <strong>{{ $reestructuracion->prestamoOriginal->cliente->nombre_completo }}</strong>,
                en adelante denominado <em>"El Cliente"</em>, quienes manifiestan lo siguiente:
            </p>
            <ul class="lista">
                <li>Que existe un crédito previamente celebrado identificado con el folio <strong>#{{ $reestructuracion->prestamo_original_id }}</strong>.</li>
        ...
        <div class="section">
            ...
            <ul class="lista">
                <li><strong>Nombre:</strong> {{ $reestructuracion->prestamoOriginal->cliente->nombre_completo }}</li>
                <li><strong>Teléfono:</strong> {{ $reestructuracion->prestamoOriginal->cliente->telefono ?? 'No especificado' }}</li>
                <li>
                    <strong>Documento:</strong>
                    @if($reestructuracion->prestamoOriginal->cliente->documento_tipo)
                        {{ strtoupper($reestructuracion->prestamoOriginal->cliente->documento_tipo) }}
                        — {{ $reestructuracion->prestamoOriginal->cliente->documento_numero ?? 'Sin número' }}
                    @else
                        No especificado
                    @endif
                </li>
                <li><strong>Domicilio:</strong> {{ $reestructuracion->prestamoOriginal->cliente->direccion ?? 'No especificado' }}</li>
            </ul>
        </div>
        ...
        <div class="section">
            ...
            <ul class="lista">
                <li><strong>Monto original:</strong> ${{ number_format($reestructuracion->prestamoOriginal->monto_original, 2) }}</li>
                <li><strong>Saldo al momento de reestructurar:</strong> ${{ number_format($reestructuracion->saldo_al_reestructurar, 2) }}</li>
                <li><strong>Mora acumulada:</strong> ${{ number_format($reestructuracion->mora_original, 2) }}</li>
                <li><strong>Tasa de interés:</strong> {{ $reestructuracion->prestamoOriginal->interes_rate }}% mensual</li>
                <li><strong>Frecuencia de pago:</strong> {{ ucfirst($reestructuracion->prestamoOriginal->frecuencia_pago) }}</li>
            </ul>
        </div>
        ...
        <div class="section">
            ...
            @if($reestructuracion->tipo === 'condonacion')
                <p>Se acuerda una <strong>condonación parcial de mora</strong>, bajo los siguientes términos:</p>
                <div class="condiciones-box">
                    <p><strong>Mora original:</strong> ${{ number_format($reestructuracion->mora_original, 2) }}</p>
                    <p><strong>Monto condonado:</strong> ${{ number_format($reestructuracion->mora_condonada, 2) }}</p>
                    <p><strong>Mora restante a cubrir:</strong> ${{ number_format($reestructuracion->mora_restante, 2) }}</p>
                    <p><strong>Saldo de capital pendiente:</strong> ${{ number_format($reestructuracion->saldo_al_reestructurar, 2) }}</p>
                </div>
                <p>El Cliente se obliga a cubrir el saldo restante conforme a las nuevas condiciones pactadas.</p>

            @elseif($reestructuracion->tipo === 'extension')
                <p>Se acuerda una <strong>extensión del plazo del crédito</strong>, en los siguientes términos:</p>
                <div class="condiciones-box">
                    <p><strong>Periodos anteriores:</strong> {{ $reestructuracion->periodos_anteriores }}</p>
                    <p><strong>Nuevos periodos:</strong> {{ $reestructuracion->periodos_nuevos }}</p>
                    <p><strong>Frecuencia de pago:</strong> {{ ucfirst($reestructuracion->prestamoOriginal->frecuencia_pago) }}</p>
                    <p><strong>Saldo pendiente:</strong> ${{ number_format($reestructuracion->saldo_al_reestructurar, 2) }}</p>
                    <p><strong>Mora congelada:</strong> ${{ number_format($reestructuracion->mora_original, 2) }}</p>
                </div>
                <p>La mora acumulada se mantiene congelada y el crédito continuará vigente bajo el nuevo calendario acordado.</p>

            @elseif($reestructuracion->tipo === 'nuevo_prestamo')
                ...
                @if($reestructuracion->prestamoNuevo)
                    <div class="condiciones-box">
                        <p><strong>Préstamo original cerrado:</strong> Folio #{{ $reestructuracion->prestamo_original_id }}</p>
                        <p><strong>Nuevo préstamo folio:</strong> #{{ $reestructuracion->prestamo_nuevo_id }}</p>
                        <p><strong>Nuevo monto:</strong> ${{ number_format($reestructuracion->prestamoNuevo->monto_original, 2) }}</p>
                        <p><strong>Tasa de interés:</strong> {{ $reestructuracion->prestamoNuevo->interes_rate }}% mensual</p>
                        <p><strong>Frecuencia:</strong> {{ ucfirst($reestructuracion->prestamoNuevo->frecuencia_pago) }}</p>
                        <p><strong>Periodos:</strong> {{ $reestructuracion->prestamoNuevo->numero_periodos }}</p>
                    </div>
                @endif
            @endif
        </div>
        ...
        <div class="section">
            ...
            <div class="motivo-box">{{ $reestructuracion->motivo }}</div>
        </div>

        @if($reestructuracion->observaciones)
            <hr class="divider">
            <div class="section">
                <p class="section-title">VI. Observaciones adicionales</p>
                <div class="motivo-box">{{ $reestructuracion->observaciones }}</div>
            </div>
        @endif
        ...
        {{-- VII. Cláusulas --}}
        <div class="section">
            <p class="section-title">{{ $reestructuracion->observaciones ? 'VII' : 'VI' }}. Cláusulas</p>
            ...
            <div class="clausula">
                ...
                En caso de incumplimiento, se generarán intereses moratorios conforme a las políticas internas del Prestamista.</p>
            </div>
            ...
            <div class="clausula">
                ...
                tribunales competentes en el estado que corresponda, renunciando a cualquier otro fuero que pudiera
                corresponderles.</p>
            </div>
        </div>
        ...
        <div class="firmas">
            ...
            <div class="firma-col">
                <div class="firma-linea">
                    <div class="firma-nombre">{{ $reestructuracion->prestamoOriginal->cliente->nombre_completo }}</div>
                    <div class="firma-cargo">El Cliente — Firma</div>
                    @if($reestructuracion->prestamoOriginal->cliente->documento_tipo)
                        <div class="firma-doc">
                            {{ strtoupper($reestructuracion->prestamoOriginal->cliente->documento_tipo) }}:
                            {{ $reestructuracion->prestamoOriginal->cliente->documento_numero ?? '—' }}
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="footer">
        <div class="footer-left">
            <p>Generado el {{ now()->format('d/m/Y \a \l\a\s H:i') }} por {{ $reestructuracion->registradoPor->name }}</p>
