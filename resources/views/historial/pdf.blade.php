...
<body>
    ...
    <div class="header">
        ...
        <div class="header-right">
            <div class="folio">
                Folio
                <span>#{{ str_pad($prestamo->id, 6, '0', STR_PAD_LEFT) }}</span>
            </div>
    ...
    <div class="body">
                ...
                <div class="section">
                    ...
                    <table class="info-table">
                        ...
                        <tr>
                            <td>Documento</td>
                            <td>
                                @if($prestamo->cliente->documento_tipo)
                                    {{ strtoupper($prestamo->cliente->documento_tipo) }}
                                    — {{ $prestamo->cliente->documento_numero ?? 'Sin número' }}
                                @else
                                    —
                                @endif
                            </td>
                ...
                <div class="section">
                    ...
                    <table class="info-table">
                        <tr>
                            ...
                            <td>{{ ucfirst($prestamo->tipo) }}</td>
                        </tr>
                        <tr>
                            <td>Monto prestado</td>
                            <td>${{ number_format($prestamo->monto_original, 2) }}</td>
                        </tr>
                        <tr>
                            <td>Interés mensual</td>
                            <td>{{ $prestamo->interes_rate }}%</td>
                        </tr>
                        <tr>
                            <td>Frecuencia de pago</td>
                            <td>{{ ucfirst($prestamo->frecuencia_pago) }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>{{ $prestamo->updated_at->format('d/m/Y') }}</td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <table class="metricas">
                <tr>
                    <td class="metrica">
                        ...
                        <span class="metrica-valor azul">${{ number_format($totalPagado, 2) }}</span>
                    </td>
                    <td class="metrica">
                        <span class="metrica-label">Capital prestado</span>
                        <span class="metrica-valor">${{ number_format($prestamo->monto_original, 2) }}</span>
                    </td>
                    <td class="metrica">
        ...
        {{-- Historial de pagos --}}
        <div class="section">
            <p class="section-title">Historial de pagos ({{ $prestamo->pagos->count() }} movimientos)</p>
            <table class="pagos-table">
                ...
                <tbody>
                    @foreach($prestamo->pagos->sortBy('fecha_pago') as $i => $pago)
                        <tr>
                            <td style="color:#aaa;">{{ $i + 1 }}</td>
                            <td>{{ $pago->fecha_pago->format('d/m/Y') }}</td>
                            <td style="font-weight:bold;">${{ number_format($pago->monto_pagado, 2) }}</td>
                            <td><span class="tipo-badge">{{ ucfirst(str_replace('_', ' ', $pago->tipo_pago)) }}</span></td>
                            <td>${{ number_format($pago->abono_capital, 2) }}</td>
                            <td>${{ number_format($pago->abono_interes, 2) }}</td>
                            @if($totalMora > 0) <td>${{ number_format($pago->abono_mora, 2) }}</td> @endif
                            <td style="color:#888;">{{ $pago->observaciones ?? '—' }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
        ...
        <div class="nota">
            ...
            <strong>#{{ str_pad($prestamo->id, 6, '0', STR_PAD_LEFT) }}</strong>.
