...
<div class="row g-3 mb-4">
    <div class="col-md-3">
        <div class="p-4 rounded-3 bg-white border" style="border-color:#e8e8e8 !important;">
            ...
            <h3 class="fw-medium mb-0" style="color:var(--color-primary); font-size:24px;">${{ number_format($totalCobrado, 2) }}</h3>
            <span class="text-muted" style="font-size:12px;">{{ $payments->count() }} pagos</span>
        </div>
    </div>
    <div class="col-md-3">
        <div class="p-4 rounded-3 bg-white border" style="border-color:#e8e8e8 !important;">
    ...
    <div class="col-md-3">
        <div class="p-4 rounded-3 bg-white border" style="border-color:#e8e8e8 !important;">
            ...
            <h3 class="fw-medium mb-0" style="color:var(--color-primary); font-size:24px;">${{ number_format($totalinterest, 2) }}</h3>
        </div>
    </div>
    <div class="col-md-3">
                ...
                @foreach($poradvisor as $advisorId => $paymentsPoradvisor)
                    <tr style="border-top:0.5px solid #f0f0f0;">
                        <td class="px-4 py-3" style="color:#1a2e1a; font-weight:500;">
                            {{ $paymentsPoradvisor->first()->recordedBy?->name ?? 'Sin asesor' }}
                        </td>
...
<div class="bg-white border rounded-3 overflow-hidden" style="border-color:#e8e8e8 !important;">
    <div class="px-4 py-3 border-bottom" style="border-color:#f0f0f0 !important;">
        <span class="fw-medium" style="font-size:14px; color:#1a2e1a;">
            Detalle de pagos — {{ \Carbon\Carbon::parse($fecha)->translatedFormat('d/m/Y') }}
        </span>
    </div>
        ...
        @foreach($payments as $payment)
            <div class="px-4 py-3 border-bottom" style="border-color:#f8f8f8 !important;">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <span style="font-size:15px; color:var(--color-primary); font-weight:500;">
