...
<div class="mb-4">
    <a href="{{ route('advisors.index') }}" class="text-muted" style="font-size:13px; text-decoration:none;">
        ← Volver a asesores
    </a>
</div>
...
<div class="bg-white border rounded-3 p-4" style="max-width:540px; border-color:#e8e8e8 !important;">
    ...
    <h6 class="fw-medium mb-4" style="color:#1a2e1a;">Editar asesor — {{ $advisor->name }}</h6>

    <form method="POST" action="{{ route('advisors.update', $advisor) }}">
        @csrf @method('PUT')
        @include('advisors._form', ['advisor' => $advisor])
        ...
        <div class="d-flex gap-2 mt-4">
            ...
            <a href="{{ route('advisors.index') }}" class="btn btn-sm"
