                ...
                <div class="col-12">
                    <div class="d-flex gap-3 flex-wrap">
                        <label class="d-flex align-items-start gap-2 p-3 rounded-3"
                            style="border:0.5px solid #ddd; cursor:pointer; min-width:180px;" id="card_penalty_none">
                        ...
                        <label class="d-flex align-items-start gap-2 p-3 rounded-3"
                            style="border:0.5px solid #ddd; cursor:pointer; min-width:180px;" id="card_penalty_fixed">
                            ...
                            <div>
                                ...
                                <span style="font-size:11px; color:#888;">$X por día (hasta el límite del periodo)</span>
                            </div>
                        </label>
                        <label class="d-flex align-items-start gap-2 p-3 rounded-3"
                            style="border:0.5px solid #ddd; cursor:pointer; min-width:180px;" id="card_penalty_pct_period">
                            <input type="radio" name="penalty_type" value="percentage_period" style="margin-top:2px;">
                            <div>
                                <span class="fw-medium d-block" style="font-size:13px; color:#1a2e1a;">% sobre saldo restante</span>
                                <span style="font-size:11px; color:#888;">X% del saldo pendiente por periodo vencido</span>
                            </div>
                        </label>
                        <label class="d-flex align-items-start gap-2 p-3 rounded-3"
                            style="border:0.5px solid #ddd; cursor:pointer; min-width:180px;" id="card_penalty_pct_balance">
                            <input type="radio" name="penalty_type" value="percentage_balance" style="margin-top:2px;">
                            <div>
                                <span class="fw-medium d-block" style="font-size:13px; color:#1a2e1a;">% sobre monto original</span>
                                <span style="font-size:11px; color:#888;">X% del préstamo total por periodo vencido</span>
                            </div>
                        </label>
                    </div>
                </div>
    ...
    function disableOtherForms(activeType) {
        ['term', 'interest', 'daily'].forEach(t => {
            const inputs = document.getElementById('form_' + t).querySelectorAll('input, select');
            inputs.forEach(input => { input.disabled = (t !== activeType); });
        });
    }
    ...
    function updatePenalty() {
        ...
        document.getElementById('card_penalty_none').style.borderColor        = checked === ''                   ? green  : grey;
        document.getElementById('card_penalty_fixed').style.borderColor       = checked === 'fixed'              ? orange : grey;
        document.getElementById('card_penalty_pct_period').style.borderColor  = checked === 'percentage_period'  ? orange : grey;
        document.getElementById('card_penalty_pct_balance').style.borderColor = checked === 'percentage_balance' ? orange : grey;

        if (checked === 'fixed') {
            document.getElementById('penalty_value_label').textContent = 'Monto por día ($)';
            document.getElementById('penalty_value_hint').textContent  = 'Ej: 20 → cobra $20/día hasta agotar el periodo';
        } else if (checked === 'percentage_period') {
            document.getElementById('penalty_value_label').textContent = 'Porcentaje (%)';
            document.getElementById('penalty_value_hint').textContent  = 'Ej: 5 → cobra 5% del saldo restante por periodo vencido';
        } else if (checked === 'percentage_balance') {
            document.getElementById('penalty_value_label').textContent = 'Porcentaje (%)';
            document.getElementById('penalty_value_hint').textContent  = 'Ej: 5 → cobra 5% del monto original por periodo vencido';
        }
    }
